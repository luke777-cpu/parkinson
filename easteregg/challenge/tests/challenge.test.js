...
const SHARED_SRC=(function(){
  const p=path.join(ROOT,"..","shared-profile.js");
  if(fs.existsSync(p)) return fs.readFileSync(p,"utf-8");
  /* 단독 폴더 실행용 동일 API 스텁 */
  return `(function(root){const S={KEY:"parkinsonSharedProfileV1"};
    S.empty=()=>({version:1,language:"ko",fontScale:1,zoomEnabled:true,primarySymptoms:[],medications:[],challengeSummaries:[],pendingDiaryEvents:[],updatedAt:""});
    S.read=function(){try{const r=localStorage.getItem(S.KEY);if(r){const d=JSON.parse(r);if(d&&d.version===1)return {...S.empty(),...d};}}catch(e){}return S.empty();};
    S.write=function(p){try{const c=S.read();const n={...c,...p,version:1,updatedAt:new Date().toISOString()};localStorage.setItem(S.KEY,JSON.stringify(n));return n;}catch(e){return null;}};
    S.pushDiaryEvent=function(ev){const c=S.read();c.pendingDiaryEvents=(c.pendingDiaryEvents||[]).concat([ev]).slice(-20);return S.write({pendingDiaryEvents:c.pendingDiaryEvents});};
    S.drainDiaryEvents=function(){const c=S.read();const q=c.pendingDiaryEvents||[];if(q.length)S.write({pendingDiaryEvents:[]});return q;};
    S.putChallengeSummary=function(s){const c=S.read();const l=(c.challengeSummaries||[]).filter(x=>x.id!==s.id);l.push(s);l.sort((a,b)=>String(a.date).localeCompare(String(b.date)));return S.write({challengeSummaries:l.slice(-5)});};
    root.SHARED=S;})(window);`;
})();
function inline(html){
  return html
    .replace('<script src="../shared-profile.js" onerror="/* 단독 배포 시 없음 — 무시 */"></script>', "<script>"+SHARED_SRC+"<\/script>")
...
setTimeout(()=>{
  ...
  ok(APP().includes("결과 저장 위치")&&APP().includes("본체 복용약·기록은 수정되지 않습니다"), "저장 위치 선택 화면(챌린지만/약효일지에도)");
  { const spBefore=JSON.parse(w.localStorage.getItem("parkinsonSharedProfileV1")||"{}");
    ok((spBefore.challengeSummaries||[]).length===1, "완료 시 PHS용 요약 자동 저장"); }
  d.getElementById("saveBoth").click();
  { const sp=JSON.parse(w.localStorage.getItem("parkinsonSharedProfileV1"));
    const ev=(sp.pendingDiaryEvents||[])[0];
    ok(ev && ev.challengeTestId===saved.tests[0].id && ev.lines.length>0 && ev.title.includes("Medication Challenge"), "약효일지 전달 큐에 요약 적재"); }
  ...
  setTimeout(()=>{
    ...
    setTimeout(()=>{
      ...
      ok(dom3.window.document.querySelector('a[href="../index.html"]'), "약효일지로 돌아가기 버튼");
      /* Shared Profile 적용: 글씨 크기·복용약 가져오기·주증상 미리 선택 + 해시 라우팅 */
      const sp={version:1, language:"ko", fontScale:1.3,
        primarySymptoms:[{type:"tremor",active:true},{type:"slow_hand",active:true}],
        medications:[{name:"마도파",dose:100,sched:["08:00"],note:""}],
        pendingDiaryEvents:[], challengeSummaries:[]};
      const cur2=w.eval("JSON.stringify(db)");
      const dom4=new JSDOM(html,{runScripts:"dangerously",url:"https://example.org/challenge/#test="+encodeURIComponent(JSON.parse(cur2).tests[0].id),
        beforeParse(w4){ w4.localStorage.setItem("medicationChallengeDbV2", cur2);
          w4.localStorage.setItem("parkinsonSharedProfileV1", JSON.stringify(sp));
          w4.confirm=()=>true; w4.alert=()=>{}; }});
      setTimeout(()=>{
        const w4=dom4.window, d4=w4.document;
        ok(d4.documentElement.style.getPropertyValue("--fs")==="1.3", "Shared 글씨 크기 적용(--fs 1.3)");
        const ml4=JSON.parse(w4.localStorage.getItem("medicationChallengeMedicationListV2"));
        ok(ml4.meds.some(m=>m.name==="마도파"&&m.usualTimes[0]==="08:00"), "본체 복용약 사본 가져오기");
        ok(d4.getElementById("app").textContent.includes("약효 비교 테스트 결과"), "해시 #test=id → 결과 화면 직행");
        w4.viewSetup();
        const pre=[...d4.querySelectorAll(".sy_grp .chip.sel")].map(b=>b.dataset.k).sort().join();
        ok(pre==="hand_movement,tremor", "주증상 → 챌린지 증상 미리 선택(slow_hand→hand_movement)");
        ...
        ok(/medication-challenge-v3/.test(sw)&&/challenge-sim\.js/.test(sw)&&/icon-192\.png/.test(sw), "SW 캐시 v3·에셋(아이콘 포함) 갱신");
