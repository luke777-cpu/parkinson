/* ============================================================
   analysis-clinical.js — 기간 기반 기록 분석 엔진 (v2.0 Phase 0 복귀)
   책임: 최근 N일 실제 기록에서 같은 성격의 복용을 그룹으로 묶고,
   각 복용 사례의 ON latency·유효 ON 도달·최고 출력을 계산해
   delayed ON / incomplete ON / ON failure "후보"를 분류하고,
   그 분류가 애초에 믿을 만한지(재현성)를 먼저 판정한다.

   Phase 0 설계 원칙 (치료구간_과다과소 설계도 참조):
   - 이 파일은 곡선 모델을 전혀 쓰지 않는다. 실측 기록만으로 성립한다.
   - 재현성이 낮으면(reproducibility) 이후 단계(역치 추정 등)로
     넘어가서는 안 된다 — 호출부가 이 판정을 보고 막아야 한다.
...
CLIN.iqr = function(values){
  const a=values.filter(v=>v!=null&&!Number.isNaN(v)).slice().sort((x,y)=>x-y);
  if(a.length<2) return null;
  const q1=quartile(a,0.25), q3=quartile(a,0.75);
  return q3-q1;
}

/* ---- Phase 0: 재현성 판정 ----
   같은 복용의 반응(ON latency)이 여러 날 걸쳐 얼마나 일관되는지를 본다.
   여기서 poor/insufficient가 나오면 역치 추정 등 이후 단계로 넘어가면 안 된다
   (호출부가 이 판정을 보고 막아야 함 — 이 함수 자체는 막지 않는다). */
CLIN.REPRO_THRESHOLDS = { insufficientN:3, poorRatio:0.60, moderateRatio:0.30 };
CLIN.reproducibility = function(episodes, opts){
  const th=Object.assign({}, CLIN.REPRO_THRESHOLDS, opts||{});
  const reached=(episodes||[]).filter(e=>e.onLatencyMin!=null);
  const lat=reached.map(e=>e.onLatencyMin).sort((a,b)=>a-b);
  const n=lat.length;
  const reasons=[];
  if(n<th.insufficientN){
    reasons.push(`분석 가능한 사례가 ${n}회로 부족합니다 (최소 ${th.insufficientN}회 필요)`);
    return {level:"insufficient", n, medianOnLatencyMin:median(lat), range:null, iqr:null, iqrRatio:null, reasons};
  }
  const med=median(lat);
  const iqrVal=CLIN.iqr(lat);
  const ratio = (med&&med>0)? iqrVal/med : null;
  let level;
  if(ratio==null){ level="insufficient"; reasons.push("중앙값을 계산할 수 없습니다"); }
  else if(ratio>th.poorRatio){ level="poor"; reasons.push(`ON 도달 시간의 편차가 큽니다 (사분위범위 ${Math.round(iqrVal)}분, 중앙값의 ${Math.round(ratio*100)}%)`); }
  else if(ratio>th.moderateRatio){ level="moderate"; reasons.push(`ON 도달 시간의 편차가 다소 있습니다 (중앙값의 ${Math.round(ratio*100)}%)`); }
  else { level="good"; }
  return {
    level, n, medianOnLatencyMin:med,
    range:[lat[0], lat[lat.length-1]], iqr:iqrVal, iqrRatio:ratio, reasons,
  };
};
